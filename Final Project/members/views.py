...
def signup(request):
    if request.method == "POST":
        ...
        if (User.objects.filter(username=username).exists()):
            messages.add_message(request, messages.INFO, 'Username already exists. Please try a different Username.')
            return redirect("/account/signup/")

        if(not(len(pass1)>7 and not pass1.isalnum())):
            messages.add_message(request, messages.INFO, 'Password must contain atleast 8 characters and should contain a symbol.')
            return redirect("/account/signup/")
        ...
        if(not(pass1==pass2)):
            ...
            try:
                ...
                newUser.save()
                messages.add_message(request, messages.SUCCESS, 'Your account has been created successfully! Please Log In to proceed.')
                return redirect("/account/login/")
